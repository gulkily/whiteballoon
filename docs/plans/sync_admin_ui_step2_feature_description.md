...
- Inline forms/modals to create/edit/remove peers (writing to `sync_peers.txt`). Validation should prevent conflicts and require tokens for hub peers.
- Buttons to trigger push/pull actions per peer; backend should run the existing CLI helpers asynchronously and surface a toast/status update.
...
3. Admin clicks “Add peer” or edits an existing one via modal form; upon save, the table refreshes.
